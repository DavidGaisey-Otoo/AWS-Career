/**
 * DocReviewPanel.jsx — renders the 2-agent documentation review.
 */

import { useMemo, useState } from 'react';
import {
  AlertCircle, AlertTriangle, BookOpen, CheckCircle2, ChevronDown, ChevronUp,
  Info, Users,
} from 'lucide-react';
import { runDocReview } from '../../lib/docAgents/master.js';
...
export function DocReviewPanel({
  docText = '', docType = 'handover', services = [], className = '',
}) {
  const review = useMemo(
    () => runDocReview({ docText, docType, services }),
    [docText, docType, services]
  );
    ...
    <div className={cn('surface rounded-2xl p-5 space-y-4', className)}>
      ...
      <div className={cn(
        ...
        {review.criticalCount > 0 && (
          <><strong className="block mb-0.5">{review.criticalCount} critical handover blocker(s)</strong>
          <span className="opacity-90">Don\'t hand over until these are fixed — the new owner can\'t operate without them.</span></>
        )}
        {review.criticalCount === 0 && review.highCount > 0 && (
          <><strong className="block mb-0.5">{review.highCount} high-impact gap(s)</strong>
          <span className="opacity-90">Doc is missing things on-call engineers will need.</span></>
        )}
        {review.criticalCount === 0 && review.highCount === 0 && (
          <><strong className="block mb-0.5">Documentation is solid</strong>
          <span className="opacity-90">No critical or high gaps. Review the medium items for polish.</span></>
        )}
      </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="text-center py-6 opacity-65 text-[12.5px]">
            {filter === 'all' ? 'No findings — doc looks clean across both agents.' : `No ${filter}-severity findings.`}
          </div>
      ...
      <div className="text-[10.5px] opacity-55 italic leading-relaxed pt-2 border-t border-token">
        ...
        doc gaps — missing sections, ambiguous regions, IAM red flags, leaked secrets, placeholder URLs.
        For deeper accuracy checks (verifying commands actually work), upgrade to LLM mode.
      </div>
    </div>
  );
}
