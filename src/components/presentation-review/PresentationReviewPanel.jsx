/**
 * PresentationReviewPanel.jsx — renders the 2-agent presentation review.
 */

import { useMemo, useState } from 'react';
import {
  AlertCircle, AlertTriangle, CheckCircle2, ChevronDown, ChevronUp,
  Info, Presentation, Users,
} from 'lucide-react';
import { runPresentationReview } from '../../lib/presentationAgents/master.js';
...
export function PresentationReviewPanel({
  slides = [], brief = '', audience = 'client', className = '',
}) {
  const review = useMemo(
    () => runPresentationReview({ slides, brief, audience }),
    [slides, brief, audience]
  );
    ...
    <div className={cn('surface rounded-2xl p-5 space-y-4', className)}>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <div className="text-[10px] font-extrabold uppercase tracking-widest text-aws-orange mb-1">
            PRES-01 · 2-agent review
          </div>
          <h3 className="text-[15px] font-extrabold flex items-center gap-2">
            <Presentation size={15} className="text-aws-orange" />
            2 presentation experts reviewed your deck
          </h3>
          <p className="text-[11.5px] opacity-70 mt-0.5">
            Liam O&apos;Brien (Narrative) · Maya Lindqvist (Visual Clarity)
      ...
      <div className={cn(
        ...
        {review.criticalCount > 0 && (
          <><strong className="block mb-0.5">{review.criticalCount} critical issue(s)</strong>
          <span className="opacity-90">Deck is missing core elements clients expect. Fix before pitching.</span></>
        )}
        {review.criticalCount === 0 && review.highCount > 0 && (
          <><strong className="block mb-0.5">{review.highCount} high-impact issue(s)</strong>
          <span className="opacity-90">Decent foundation but missing things that close deals.</span></>
        )}
        {review.criticalCount === 0 && review.highCount === 0 && (
          <><strong className="block mb-0.5">Deck is solid</strong>
          <span className="opacity-90">No critical or high issues. Review medium items for polish.</span></>
        )}
      </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="text-center py-6 opacity-65 text-[12.5px]">
            {filter === 'all' ? 'No findings — deck looks clean across both agents.' : `No ${filter}-severity findings.`}
          </div>
      ...
      <div className="text-[10.5px] opacity-55 italic leading-relaxed pt-2 border-t border-token">
        <strong>Honest scope:</strong> Rule engines, not LLMs. Catch ~80% of common
        deck problems — missing sections, ordering issues, dense slides, weak titles.
        For voice/tone polish, upgrade to LLM mode.
      </div>
    </div>
  );
}
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
              ...
              {finding.slideIndex != null && (
                <span className="text-[10px] opacity-65">· slide {finding.slideIndex + 1}</span>
              )}
            </div>
