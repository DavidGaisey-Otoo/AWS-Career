/**
 * AgentReviewPanel.jsx — shared review panel used by deploy / contract /
 * invoice / doc / presentation / email / proposal panels.
 *
 * Takes a pre-computed `review` (output of any runXxxReview function) and
 * renders the standard grade card + severity chips + per-agent breakdown
 * + findings list.
 *
 * Each domain-specific panel is now a thin wrapper that computes its review
 * and passes it in, instead of duplicating 200 lines of rendering code.
 */
...
export function AgentReviewPanel({
  ...
  const banner =
    review.criticalCount > 0
      ? (bannerCopy.critical || `${review.criticalCount} critical issue(s) — must fix before sending.`)
      : review.highCount > 0
        ? (bannerCopy.high || `${review.highCount} high-impact issue(s) — strongly recommend fixing.`)
        : (bannerCopy.clean || 'No critical or high issues. Review medium items for polish.');
  ...
  return (
    <div className={cn('surface rounded-2xl p-5 space-y-4', className)}>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <h3 className="text-[15px] font-extrabold flex items-center gap-2">
            {Icon && <Icon size={15} className="text-aws-orange" />}
            {title}
          </h3>
          {subtitle && (
            <p className="text-[11.5px] opacity-70 mt-0.5">{subtitle}
      ...
      <div className={cn('rounded-xl border p-3 text-[12.5px]', bannerTone)}>
        {banner}
      </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="text-center py-6 opacity-65 text-[12.5px]">
            {filter === 'all' ? 'No findings — looks clean across all agents.' : `No ${filter}-severity findings.`}
          </div>
