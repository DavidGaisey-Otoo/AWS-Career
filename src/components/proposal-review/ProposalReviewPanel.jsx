/**
 * ProposalReviewPanel.jsx — renders the 3-agent proposal review.
 */
...
import { runProposalReview } from '../../lib/proposalAgents/master.js';
...
export function ProposalReviewPanel({
  proposalText = '', jobBrief = '', clientName = '',
  level = 'intermediate', services = [], className = '',
}) {
  const review = useMemo(
    () => runProposalReview({ proposalText, jobBrief, clientName, level, services }),
    [proposalText, jobBrief, clientName, level, services]
  );
    ...
    <div className={cn('surface rounded-2xl p-5 space-y-4', className)}>
      ...
      <div className={cn(
        ...
        {review.criticalCount > 0 && (
          ...
          <span className="opacity-90">This proposal will likely lose the gig. Fix the critical items before sending.</span></>
        )}
        {review.criticalCount === 0 && review.highCount > 0 && (
          <><strong className="block mb-0.5">{review.highCount} high-impact issue(s)</strong>
          <span className="opacity-90">Decent foundation but missing things that win projects. Address the high items.</span></>
        )}
        {review.criticalCount === 0 && review.highCount === 0 && (
          <><strong className="block mb-0.5">Proposal is solid</strong>
          <span className="opacity-90">No critical or high issues. Review medium items for extra polish.</span></>
        )}
      </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="text-center py-6 opacity-65 text-[12.5px]">
            {filter === 'all'
              ? 'No findings — proposal looks clean across all 3 agents.'
      ...
      <div className="text-[10.5px] opacity-55 italic leading-relaxed pt-2 border-t border-token">
        ...
        proposal mistakes (weak hooks, no pricing/timeline, jargon, weak words, typos).
        For full voice rewrites, upgrade to LLM mode.
      </div>
    </div>
  );
}
