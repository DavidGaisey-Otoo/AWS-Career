import { accountHygieneAuditor } from './hygiene.js';
import { bySeverity, buildContext, gradeFromScore, scoreFromFindings, SEVERITY } from './framework.js';

export const ALL_ACCT_AGENTS = [accountHygieneAuditor];

export function runAccountHygieneReview(input = {}) {
  const ctx = buildContext(input);

  const perAgent = ALL_ACCT_AGENTS.map((agent) => {
    let findings = [];
    try { findings = agent.review(ctx) || []; }
    catch (err) { /* eslint-disable-next-line no-console */ console.error(`[acct] ${agent.id}:`, err); }
  ...
  return {
    findings: combined, perAgent, score, grade,
