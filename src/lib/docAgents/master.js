/**
 * docAgents/master.js — orchestrator for documentation quality agents.
 *
 * runDocReview({ docText, docType, services }) → { findings[], score, ... }
 */

import { handoverCompleteness } from './completeness.js';
import { technicalAccuracy } from './accuracy.js';
import { bySeverity, buildContext, gradeFromScore, scoreFromFindings, SEVERITY } from './framework.js';

export const ALL_DOC_AGENTS = [handoverCompleteness, technicalAccuracy];

export function runDocReview(input = {}) {
  const ctx = buildContext(input);

  const perAgent = ALL_DOC_AGENTS.map((agent) => {
    let findings = [];
    try {
      findings = agent.review(ctx) || [];
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error(`[docAgents] ${agent.id} threw:`, err);
    }
    return {
      id: agent.id,
      name: agent.name,
