/**
 * deployAgents/master.js — orchestrator for deploy safety agents.
 *
 * runDeployReview({ template, format, services, region }) → review
 */

import { deployPreFlightAuditor } from './preFlight.js';
import { costBlastRadiusEstimator } from './blastRadius.js';
import { bySeverity, buildContext, gradeFromScore, scoreFromFindings, SEVERITY } from './framework.js';

export const ALL_DEPLOY_AGENTS = [deployPreFlightAuditor, costBlastRadiusEstimator];

export function runDeployReview(input = {}) {
  const ctx = buildContext(input);

  const perAgent = ALL_DEPLOY_AGENTS.map((agent) => {
    let findings = [];
    try {
      findings = agent.review(ctx) || [];
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error(`[deployAgents] ${agent.id} threw:`, err);
    }
    return {
      id: agent.id,
      name: agent.name,
