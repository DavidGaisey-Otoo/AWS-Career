/**
 * contractAgents/master.js — orchestrator for contract review agents.
 *
 * runContractReview({ contractText, clientName, currency, jurisdiction }) → review
 */

import { legalProtectionLawyer } from './legal.js';
import { paymentTermsAuditor } from './paymentTerms.js';
import { bySeverity, buildContext, gradeFromScore, scoreFromFindings, SEVERITY } from './framework.js';

export const ALL_CONTRACT_AGENTS = [legalProtectionLawyer, paymentTermsAuditor];

export function runContractReview(input = {}) {
  const ctx = buildContext(input);

  const perAgent = ALL_CONTRACT_AGENTS.map((agent) => {
    let findings = [];
    try {
      findings = agent.review(ctx) || [];
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error(`[contractAgents] ${agent.id} threw:`, err);
    }
    return {
      id: agent.id,
      name: agent.name,
