/**
 * invoiceAgents/master.js — orchestrator for invoice review.
 *
 * runInvoiceReview({ invoice, supplierCountry, clientCountry }) → review
 */

import { invoiceQualityAuditor } from './quality.js';
import { bySeverity, buildContext, gradeFromScore, scoreFromFindings, SEVERITY } from './framework.js';

export const ALL_INVOICE_AGENTS = [invoiceQualityAuditor];

export function runInvoiceReview(input = {}) {
  const ctx = buildContext(input);

  const perAgent = ALL_INVOICE_AGENTS.map((agent) => {
    let findings = [];
    try {
      findings = agent.review(ctx) || [];
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error(`[invoiceAgents] ${agent.id} threw:`, err);
    }
    return {
      id: agent.id,
      name: agent.name,
