/**
 * proposalAgents/master.js — orchestrator for proposal quality agents.
 *
 * runProposalReview({ proposalText, jobBrief, clientName, level, services })
 *   → { findings[], score, grade, criticalCount, ... perAgent[] }
 */

import { persuasionArchitect } from './persuasion.js';
import { pricingStrategist } from './pricing.js';
import { brandVoiceCoach } from './brandVoice.js';
import { bySeverity, buildContext, gradeFromScore, scoreFromFindings, SEVERITY } from './framework.js';

export const ALL_PROPOSAL_AGENTS = [persuasionArchitect, pricingStrategist, brandVoiceCoach];

export function runProposalReview(input = {}) {
  const ctx = buildContext(input);

  const perAgent = ALL_PROPOSAL_AGENTS.map((agent) => {
    let findings = [];
    try {
      findings = agent.review(ctx) || [];
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error(`[proposalAgents] ${agent.id} threw:`, err);
    }
    return {
      id: agent.id,
      name: agent.name,
