/**
 * testRunner.js — runs each proposal scenario through runProposalReview.
 */

import { runProposalReview } from '../master.js';
import { PROPOSAL_SCENARIOS } from './fixtures.js';

export function runAllProposalTests() {
  const results = PROPOSAL_SCENARIOS.map((scenario) => {
    const review = runProposalReview(scenario.input);
  ...
  return {
    results,
    summary: { totalScenarios: results.length, passCount, catchRate: overallCatch, totalExpected, totalCaught },
  };
}

export function printProposalReport(report) {
  const lines = [];
  lines.push('═══════════════════════════════════════════════════════════════════');
  lines.push('  PROPOSAL AGENT TEST REPORT');
  lines.push('═══════════════════════════════════════════════════════════════════');
  lines.push('');
