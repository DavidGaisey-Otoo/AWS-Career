...
Use secure remote administration, patching, monitoring, backup, and a controlled restore.`);
  ...
  },
  {
    ...
    run() {
      const s = runPipeline('Build a short-lived AWS training lab for Windows Server administration in eu-north-1. Use one Windows EC2 instance with Systems Manager, CloudWatch, encrypted EBS, IAM, and AWS Backup. Keep spend below $5 and tear down within two hours.');
      ...
      assert(/SecurityGroupIngress: \[\]/.test(s.artifacts.cfn.code),
        'empty Windows ingress list must be emitted as valid inline YAML');
      assert(!s.artifacts.cfn.code.includes('AWS::Budgets::Budget'),
        'regional Windows lab must not include the unsupported account-global Budgets resource');
    },
  },
