...
locals {
  common_tags = {
    ...
    ManagedBy   = "Terraform"%PREPARED_BY%
  }
}
`;

/**
 * Who prepared this, for the client's benefit.
 *
 * These artifacts are handed to clients and deployed into their AWS
 * accounts, so they must not announce the tool that produced them. A
 * CloudFormation description reading "Generated by AWS Career Launchpad
 * Pro" tells a client their infrastructure came out of someone's career
 * training app, and the same string was going into their resource tags,
 * where it would sit in their console and their bill indefinitely.
 *
 * With no name configured the line is omitted entirely. Silence is
 * professional; the wrong attribution is not.
 */
function preparedBy(opts = {}) {
  const name = (opts.author || opts.businessName || '').trim();
  return name || null;
}

/** The banner at the top of a generated artifact. */
function headerLines(project, opts = {}, mode = 'test') {
  const who = preparedBy(opts);
  const out = [`${project}`];
  // The test marker stays — it is a safety signal, not decoration — but
  // without the emoji, which has no place in a client deliverable.
  if (mode === 'test') out.push('TEST DEPLOYMENT — free-tier sizing, not for production use');
  if (who) out.push(`Prepared by ${who}`);
  return out;
}
...
export function generateCloudFormation(services, opts = {}) {
  ...
  const template = {
    ...
    Description: [
      project,
      `${covered.length} service${covered.length === 1 ? '' : 's'}`,
      mode === 'test' ? 'test sizing' : null,
      preparedBy(opts) ? `prepared by ${preparedBy(opts)}` : null,
    ].filter(Boolean).join(' - '),
    Parameters: params,
    Resources: resources,
    ...(!deployReady ? {
...
export function generateCli(services, opts = {}) {
  ...
  lines.push('# ============================================================');
  for (const line of headerLines(project, opts, mode)) lines.push(`# ${line}`);
  lines.push('# ============================================================');
