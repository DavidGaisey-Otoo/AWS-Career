import { projectPlanRealismAuditor } from './realism.js';
import { bySeverity, buildContext, gradeFromScore, scoreFromFindings, SEVERITY } from './framework.js';

export const ALL_PLAN_AGENTS = [projectPlanRealismAuditor];

export function runProjectPlanReview(input = {}) {
  const ctx = buildContext(input);
  const perAgent = ALL_PLAN_AGENTS.map((agent) => {
    let findings = [];
    try { findings = agent.review(ctx) || []; }
    catch (err) { /* eslint-disable-next-line no-console */ console.error(`[plan] ${agent.id}:`, err); }
