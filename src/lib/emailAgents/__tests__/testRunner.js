/**
 * testRunner.js — runs every scenario through runEmailReview() and
 * reports catch rate per scenario + overall.
 */

import { runEmailReview } from '../master.js';
import { EMAIL_SCENARIOS } from './fixtures.js';

export function runAllEmailTests() {
  const results = EMAIL_SCENARIOS.map((scenario) => {
    const review = runEmailReview(scenario.input);
...
export function printEmailReport(report) {
  const lines = [];
  lines.push('═══════════════════════════════════════════════════════════════════');
  lines.push('  EMAIL AGENT TEST REPORT');
  lines.push('═══════════════════════════════════════════════════════════════════');
  lines.push('');
  ...
  for (const r of report.results) {
    ...
    if (r.overFlagged > 0) {
      lines.push(`    OVER-FLAGGED: ${r.overFlagged} extra findings beyond expected ceiling`);
    }
  }
